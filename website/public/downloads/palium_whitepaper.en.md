# Palium Project Whitepaper

*Derived from the PaliMesh whitepaper v0.2; covers only the public chain. For storage, identity and memory see [palimesh.io](https://palimesh.io).*
...
Palium is an EVM-compatible blockchain that innovatively combines **on-chain settlement** with **off-chain proofs** through the **PoSe v2 (Proof-of-Service v2) mechanism** to implement a **storage verification layer**.

Palium is purpose-built for the **OpenClaw AI-Agent Ecosystem**, providing:
...
Palium is also a general-purpose blockchain supporting EVM smart contracts, JSON-RPC, and WebSocket subscriptions.
...
Palium's default incentive weights favor **FN uptime/RPC**, so ordinary nodes earn meaningfully without running archives.
...
Palium allocates each epoch's reward pool into three buckets:
...
**Palium preserves verifiability by keeping AI agents strictly in the operations layer.**
...
Palium may use a decaying inflation schedule to bootstrap early participation:
...
```
Default Block Time: 1000ms (configurable)
...
| Dimension | Palium | Ethereum | Solana | Polygon |
|-----------|-----|----------|--------|---------|
...
**Key Advantage**: Palium is purpose-built for OpenClaw AI-agent infrastructure, with verifiable service proofs, automated enforcement, and closed-loop incentives.

### 18.2 vs Storage-Focused Networks

| Dimension | Palium | Filecoin | Arweave | Storj |
|-----------|-----|----------|---------|-------|
...
**Key Distinction**: Filecoin/Arweave are storage specialists; Palium integrates execution + storage + verifiable settlement.
...
| **Block Time** | 1000ms | Configurable |
