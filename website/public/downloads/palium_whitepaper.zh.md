...
*本文由 PaliMesh 白皮书 v0.2 派生，仅覆盖公链部分；存储、身份与记忆见 [palimesh.io](https://palimesh.io)。*
...
Palium 是一条 EVM 兼容的公链，创新性地将**链上结算**(On-Chain Settlement)与**链下证明**(Off-Chain Proof)结合，通过 **PoSe v2 (Proof-of-Service v2)** 机制实现**存储验证层**。

Palium 专为 **OpenClaw AI 代理生态** 设计，提供：
- **可验证的服务证明**：通过 EIP-712 签名和见证人仲裁
- **自动化结算和惩罚**：链上合约自动检测和执行
- **闭环激励**：收费、奖励和惩罚在一个协议层完成
- **AI 代理原生**：节点身份承诺、端点证明、服务能力标志集成

Palium 同时也是通用区块链，支持 EVM 智能合约、JSON-RPC、WebSocket 订阅。
...
Palium 默认激励权重偏向 **FN 正常运行时间/RPC**，所以普通节点无需运行归档也能获得意义的奖励。
...
**Palium 通过将 AI 代理严格保持在操作层来保持可验证性。**
...
| 维度 | Palium | Ethereum | Solana | Polygon |
|------|-----|----------|--------|---------|
...
**关键优势**：Palium 是专为 OpenClaw AI 代理基础设施设计的，提供可验证的服务证明、自动化执行和闭环激励。

### 18.2 与存储型公链对比

| 维度 | Palium | Filecoin | Arweave | Storj |
|------|-----|----------|---------|-------|
...
**关键区别**：Filecoin/Arweave 是存储专家；Palium 是执行 + 存储，集成可验证和结算。
